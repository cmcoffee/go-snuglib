...
func (s *Store) Get(section, key string) []string {
	...
	if result, found := s.cfgStore[section][key]; !found {
		...
		if len(result) == 0 { return []string{EMPTY} }
		return result
	}
}
	...
	for s.Scan() {
		...
		for i, ch := range txt {
			...
			case '[':
				...
				fallthrough
			case '#':
				if flag&cfg_KEY == 0 && !skip { continue scanLoop }
				if !skip { skip = true }
				fallthrough
			case '=':
				if flag&cfg_KEY == 0 && !skip {
			...
			default:
				...
				flag &^= cfg_COMMA
				if i == l-1 && buf.Len() != 0 { return out, cfgErr(file, line) }
				if flag&cfg_ESCAPE > 0 { 
					buf.WriteRune('\\')
					flag&^= cfg_ESCAPE 
				}
...
func ReadFile(file, section string) (out map[string][]string, err error) {
	...
	for s.Scan() {
		...
		if flag&cfg_HEADER == 0 {
			...
			if strings.HasPrefix(txt, "["+section+"]") { flag |= cfg_HEADER }
			...
			for i, ch := range txt {
				...
				case ',':
					...
					if flag&cfg_ESCAPE > 0 { flag &^= cfg_ESCAPE }
					if !skip { skip = true }
					fallthrough
				case '[':
					if flag&cfg_KEY == 0 && !skip {
